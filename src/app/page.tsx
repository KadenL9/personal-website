...
// Mobile Components (Clean Stack Layouts)
import ScreenAboutMobile from "../components/ScreenAboutMobile";
import ScreenProjectsMobile from "../components/ScreenProjectsMobile";
import ScreenExperienceMobile from "../components/ScreenExperienceMobile";
...
export default function Home() {
    ...
    // Safe Client-Side screen check to avoid hydration mismatch
    useEffect(() => {
        const checkScreenSize = () => {
            const isTooNarrow = window.innerWidth < 1024;
            const isTooShort = window.innerHeight < 700;
            setIsMobile(isTooNarrow || isTooShort);
        };
    ...
    // Scroll listener only active on Desktop (when scroll-locking is active)
    useMotionValueEvent(scrollYProgress, "change", (latest) => {
        if (isMobile) return; // Don't snap/switch stages on mobile!
    ...
    // Jump helper for our header nav button clicks
    const handleNavClick = (targetStage: number) => {
        if (isMobile) {
            // On mobile, find the element and smoothly scroll to its place on the document
            const element = document.getElementById(`mobile-sec-${targetStage}`);
            if (element) {
                element.scrollIntoView({ behavior: "smooth" });
            }
        } else {
            // On desktop, jump along our artificial scroll track wrapper
    ...
    return (
        /* 
           FIXED: On mobile, we swap from the tall artificial "400vh" height container 
           to a standard height ("h-auto") to allow natural, clean document scrolling.
        */
        <div className={`relative ${isMobile ? "h-auto" : "h-[400vh]"} text-foreground select-none transition-colors duration-500 ${isDarkMode ? "bg-theme-bg dark" : "bg-neutral-50 light"}`}>
            
            {/* 
               FIXED: On mobile, we swap from "sticky h-screen" to relative positioning 
               so contents don't lock onto the screen and can be scrolled past normally.
            */}
            <div className={`relative w-full flex flex-col justify-between p-6 md:p-12 ${isMobile ? "h-auto" : "sticky top-0 h-screen overflow-hidden"} z-10`}>
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className={`transition-colors duration-200 cursor-pointer pointer-events-auto hover:text-foreground ${
                                        !isMobile && stage === targetStage ? "text-theme-accent" : ""
                                    }`}
                                >
                                    {label}
                                </button>
                ...
                <main className="w-full max-w-6xl mx-auto z-10 flex flex-col lg:items-center lg:justify-center relative min-h-[500px] flex-grow mt-8 lg:mt-0">
                    {isMobile ? (
                        /* 
                           ==========================================
                           MOBILE DOCUMENT FLOW VIEW
                           All sections are stacked sequentially for 
                           a natural scroll feed.
                           ==========================================
                        */
                        <div className="w-full flex flex-col gap-16 py-8">
                            <div id="mobile-sec-0" className="w-full scroll-mt-24">
                                <ScreenAboutMobile isDarkMode={isDarkMode} />
                            </div>
                            <div id="mobile-sec-1" className="w-full scroll-mt-24">
                                <ScreenProjectsMobile projects={projects} />
                            </div>
                            <div id="mobile-sec-2" className="w-full scroll-mt-24">
                                <ScreenExperienceMobile experiences={experiences} />
                            </div>
                        </div>
                    ) : (
                        /* 
                           ==========================================
                           DESKTOP SCREEN INTERFACE
                           Single stage switching via Framer Motion.
                           ==========================================
                        */
...
// ==========================================
// BACKGROUND FLUID WAVES COMPONENT
// ==========================================
